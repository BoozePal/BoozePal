...
public class UserServiceEndpoint extends AbstractEndpoint implements Serializable {
    ...
    public Response getUserById(@PathParam("id") Long id) {
        logger.info("Felhasználó visszaadása id alapján:{} .", id);
        User user;
        try {
            user = userServiceRest.getUserById(id);
        } catch (RuntimeException e) {
            logger.error("Hiba a felhasználó lekérése közben azonosító alapján.", e);
            return Response.serverError().entity(e.getMessage()).build();
        }
        return Response.ok().entity(user).build();
    }
